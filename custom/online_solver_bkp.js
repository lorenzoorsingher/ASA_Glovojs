...
async function getResult(responseCheckUrl) {
  while (true) {
    ...
    if (res.status != 200) {
      throw new Error(
        `Received HTTP error from ${HOST + res.result} ` + (await res.text())
      );
    }
  ...
  if (json.status != "ok") {
    console.log(json);
    throw new Error(
      `Invalid 'status' in response body from ${responseCheckUrl}`
    );
  }
...
async function parsePlan(json) {
  /**@type {[string]}*/
  var lines = [];
  if (json.result.output.plan) lines = json.result.output.plan.split("\n");
